...
void D_Display (void)
{
    ...
    switch (gamestate)
    {
      case GS_LEVEL:
		...
		if (automapactive)
			AM_Drawer ();
		if (wipe || (viewheight != RENDERHEIGHT && fullscreen) )
			redrawsbar = true;
		if (inhelpscreensstate && !inhelpscreens)
			redrawsbar = true;              // just put away the help screen
		ST_Drawer (viewheight == RENDERHEIGHT, redrawsbar );
		fullscreen = viewheight == RENDERHEIGHT;
		break;
    ...
    if (gamestate == GS_LEVEL && !automapactive && scaledviewwidth != RENDERWIDTH)
    {
    ...
    if (paused)
    {
		if (automapactive)
			...
			y = viewwindowy / render_scale+4;
		V_DrawPatchDirect(viewwindowx / render_scale + (scaledviewwidth / render_scale - 68) / 2, y,
    ...
    R_BeginDisplay(); // Wipes deliberately retain the original pixel-art effect.
