...
{ 
    ...
    if (dest_screen == I_VideoBuffer) R_OverlayRect(destx, desty, width, height); 
...
{ 
    ...
    for ( ; col<w ; x++, col++, desttop++)
    {
        ...
        while (column->topdelta != 0xff)
        {
            ...
            {
                *dest = *source++;
                if (dest_screen == I_VideoBuffer) render_mask[dest - dest_screen] = 0;
                dest += SCREENWIDTH;
            }
...
{ 
    ...
    if (dest_screen == I_VideoBuffer) R_OverlayRect(x, y, width, height); 
...
void V_DrawRawScreen(byte *raw)
{
    ...
    if (dest_screen == I_VideoBuffer) R_BeginDisplay();
}
